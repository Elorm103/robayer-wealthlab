/**
 * EPUB Chapter Service - Secure Digital Library, Phase 5.
 *
 ...
 * real workerd runtime by a dedicated compatibility spike (Phase 1
 * inspection) before this file was written.
 *
 ...
 * conscious, disclosed scope reduction for the secure reader path
 * only. The existing, unmodified epub.js-based reader (used whenever
 * secure_reader_enabled is off, or for a book/product not yet using
 * this path) is completely unaffected and keeps its full TOC fidelity.
 */
...
/** Server-side mirror of library-reader.js's own injectReaderCsp(): strips any author-supplied CSP and inserts this reader's own, script-src 'none' always included - the exact same policy string, so a book delivered through the secure path is no less hardened against hostile EPUB content than the existing reader already is. */
