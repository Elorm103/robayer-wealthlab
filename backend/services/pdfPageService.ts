/**
 * PDF Page Service - Secure Digital Library, Phase 4.
 *
 ...
 * - proven to run cleanly in this project's real workerd runtime by a
 * dedicated compatibility spike (Phase 1 inspection) before this file
 * was written, and the exact same drawing primitives
 * services/orders/receiptPdfService.ts already uses in production
 * today. Deliberately NOT pixel rasterization: no canvas backend
 * exists in workerd (confirmed separately by
...
export async function renderProtectedPdfPage(masterBytes: ArrayBuffer, pageNumber: number, watermark: WatermarkInput): Promise<RenderPdfPageResult> {
  ...
  try {
    ...
    // real friction, not a one-click crop, though (per the explicit
    // instruction this makes no stronger claim than that) a
    // sufficiently motivated actor can still crop or paint over a
    // visible watermark; this is deterrence and traceability, not
