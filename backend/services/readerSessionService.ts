/**
 * Reader Session Service - Secure Digital Library, Phase 3.
 *
 * A reader session is the credential every page/chapter request in
 * the protected reader is scoped to: short-lived, revocable, bound to
...
/** Lazily generates and persists deliveries.owner_watermark_id the first time a delivery ever opens a secure reader session - never backfilled in bulk by the migration itself. A fresh, independent identifier (not derived from licenses.license_key): not every delivery is guaranteed to have a matching license row, and this stays a simple, self-contained fact about the delivery. */
