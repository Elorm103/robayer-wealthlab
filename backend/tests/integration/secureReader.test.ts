/**
 * Integration tests: the Secure Digital Library end-to-end HTTP flow.
...
const PDF_STORAGE_KEY = 'ebooks/secure-reader-test.pdf';
const EPUB_STORAGE_KEY = 'ebooks/secure-reader-test.epub';
const PDF_ASSET_ID = 'asset-secure-reader-test-pdf-v1';
const EPUB_ASSET_ID = 'asset-secure-reader-test-epub-v1';
const PDF_SLUG = 'secure-reader-test-pdf';
const EPUB_SLUG = 'secure-reader-test-epub';
...
beforeEach(async () => {
  ...
  await env.DB.exec(`DELETE FROM site_settings WHERE key = 'secure_reader_enabled'`);
  ...
  await env.DB.prepare(
    `INSERT INTO products (product_id, slug, title, topic, product_type, status, price_pesewas, currency, pricing_model, tax_behavior, language)
     VALUES ('prod-secure-reader-pdf', ?, 'Secure Reader Test PDF', 'investing', 'ebook', 'active', 3900, 'GHS', 'one-time', 'inclusive', 'en')`
  )
    .bind(PDF_SLUG)
    .run();
  await env.DB.prepare(
    `INSERT INTO products (product_id, slug, title, topic, product_type, status, price_pesewas, currency, pricing_model, tax_behavior, language)
     VALUES ('prod-secure-reader-epub', ?, 'Secure Reader Test EPUB', 'investing', 'ebook', 'active', 3900, 'GHS', 'one-time', 'inclusive', 'en')`
  )
    .bind(EPUB_SLUG)
    .run();
  ...
  const pdfMedia = await env.DB.prepare(
    `INSERT INTO media_assets (filename, original_filename, mime_type, size_bytes, content_hash, storage_key, public_url, media_type, folder, status)
     VALUES ('secure-reader-test.pdf', 'secure-reader-test.pdf', 'application/pdf', 1024, 'aaaa', ?, 'https://example.com/x.pdf', 'document', 'books', 'ready')`
  )
    .bind(PDF_STORAGE_KEY)
    .run();
  const epubMedia = await env.DB.prepare(
    `INSERT INTO media_assets (filename, original_filename, mime_type, size_bytes, content_hash, storage_key, public_url, media_type, folder, status)
     VALUES ('secure-reader-test.epub', 'secure-reader-test.epub', 'application/epub+zip', 1024, 'bbbb', ?, 'https://example.com/x.epub', 'document', 'books', 'ready')`
  )
...
async function seedCustomerWithPurchase(email: string, opts: { assetId: string; productSlug: string; reference: string; deliveryStatus?: string }) {
  ...
  const purchaseInsert = await env.DB.prepare(
    `INSERT INTO purchase_sessions (purchase_reference, product_slug, product_id, product_title, amount_pesewas, currency, status, customer_id, expires_at)
     VALUES (?, ?, 'prod-secure-reader-x', 'Secure Reader Test', 3900, 'GHS', 'verified', ?, datetime('now', '+30 minutes'))`
  )
...
describe('POST /api/customer/purchases/:reference/reader-session', () => {
  ...
  it('when secure_reader_enabled is off, minting is refused with a distinct code the client uses to fall back to the legacy flow', async () => {
    await env.DB.prepare(`INSERT INTO site_settings (key, value) VALUES ('secure_reader_enabled', 'false')`).run();
    ...
    expect(body.success).toBe(false);
    expect(body.error.code).toBe('SECURE_READER_DISABLED');
  });
...
describe('Immediate kill switch: secure_reader_enabled stops an already-active session, not just new mints', () => {
  it('a valid, unexpired PDF reader session is blocked from fetching further pages the moment the flag is turned off, with no need to wait for the session to expire', async () => {
    ...
    expect(afterBody.error.code).toBe('SECURE_READER_DISABLED');
  });
  ...
  it('re-enabling the flag restores access to a still-unexpired, unrevoked session with no new mint needed', async () => {
    ...
    await env.DB.prepare(`UPDATE site_settings SET value = 'true' WHERE key = 'secure_reader_enabled'`).run();
...
describe('Property 9: existing download behavior is unchanged, now with added audit logging', () => {
  it('GET /api/download/:token (download purpose) still returns the COMPLETE file, byte-for-byte identical to the master - the explicit download pathway is untouched', async () => {
    const purchase = await seedCustomerWithPurchase('download-unchanged@example.com', { assetId: PDF_ASSET_ID, productSlug: PDF_SLUG, reference: 'RWL-2026-900040' });
    ...
    // The one deliberate addition (Phase 8): this real download is now
    // also logged, purely additively - nothing about the response
    // above changed because of it.
    const logRow = await env.DB.prepare(`SELECT action, delivery_id AS deliveryId FROM content_access_log WHERE action = 'download'`).first<any>();
    expect(logRow).toBeTruthy();
    expect(logRow.deliveryId).toBe(purchase.deliveryId);
  });
});
...
describe('Property 10: no protected asset accidentally becomes publicly accessible', () => {
  ...
  it('a secure reader session token is never usable as a public media key or otherwise reachable without going through the reader endpoints', async () => {
