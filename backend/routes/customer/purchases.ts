...
// ============================================================
// Secure Digital Library - Reader Session (Phase 3). Customer-auth-
...
// a customer cookie is required in the whole secure-reader flow.
// ============================================================
...
export async function handleRequestReaderSession(request: Request, env: Env, logger: Logger, params: RouteParams): Promise<Response> {
  ...
  // The documented rollback path: when disabled, this endpoint simply
  // stops minting new sessions, and library-reader.js falls back to
  // its existing, unmodified whole-file read-access flow - no deploy
  // or migration needed to recover from a production issue.
  if (!(await isSecureReaderEnabled(env))) {
    return withNoStore(jsonError('SECURE_READER_DISABLED', 'The protected reader is not currently available.'));
  }
  ...
  if (!asset || (asset.fileType !== 'PDF' && asset.fileType !== 'EPUB')) {
    return withNoStore(jsonError('READER_ACCESS_DENIED', 'This resource is not available in the protected reader.'));
  }
