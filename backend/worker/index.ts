...
// Controlled Library Reader, Phase 2 - the controlled reader's content
...
import {
  ...
  handleGetAffiliateProductPerformance,
...
const ROUTES: Route[] = [
  ...
  { pattern: new URLPattern({ pathname: '/api/admin/users/:id/restore' }), method: 'POST', handler: handleRestoreAdmin },
  ...
  // Controlled Library Reader, Phase 2 - reader session mint
  // (customer-auth-gated) and the token-scoped page/chapter content
  // endpoints (matching the existing GET /api/download/:token
  // bearer-token-in-URL pattern).
  ...
  // Quick Checks / Action boxes. See routes/customer/libraryLearning.ts
  ...
  { pattern: new URLPattern({ pathname: '/api/customer/affiliates/overview/by-product' }), method: 'GET', handler: handleGetAffiliateProductPerformance },
