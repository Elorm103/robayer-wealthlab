/**
 * Controlled Library Reader - mid-session PDF error handling
 * regression tests. Real bug found during the pre-commit security
 * review of the earlier secure-reader work: the per-page shim
 * (createControlledPdfDocShim) throws when a page fetch fails - an
 * expired/revoked reader session, or the controlled_reader_enabled
 * kill switch being turned off mid-session (see routes/reader.ts's
 * flag check) - but renderPage() only cleared its `rendering` guard
 * on the SUCCESS path, and none of its three callers (goToPage,
 * setScale, refitAndRerender) caught the rejection. The result:
 * `rendering` stayed stuck `true` forever, which silently disabled
 * every further page turn/zoom/resize - no error shown, no console
 * message a customer would ever see, just a reader that stopped
 * responding. This is exactly the "unhandled promise rejection or
 * silent stall" the fix targets.
 *
