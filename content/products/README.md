...
| `downloadFiles` | An array of `{ label, path, format }`, each path under `assets/downloads/` — **never served directly to an unpaid visitor**; see `docs/download-security.md`. Sprint 2.1 changed this from a single `downloadFile` path to an array, at zero migration cost (no real data existed), to directly support a product shipping more than one file (e.g. an `.xlsx` template plus a `.pdf` instructions sheet). |
