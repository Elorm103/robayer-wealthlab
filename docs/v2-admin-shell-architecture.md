...
> **Superseded — Version 2.0 Same-Origin Migration.** The premise above ("two different hosts... were always going to be") turned out not to hold: the Worker was moved onto `robayerwealthlab.com/api/*` via a Cloudflare Workers Route, making the frontend and API genuinely same-origin. `SameSite=None` and `middleware/cors.ts` (including `Access-Control-Allow-Credentials`) have both been reverted/removed — cookies are `SameSite=Lax`, and there is no CORS layer at all. See `docs/v2-same-origin-architecture.md` for the current, permanent architecture. This section is kept as an accurate historical record of the cross-origin design that was live between Phase 0.2 and the Same-Origin Migration — not a description of the system as it exists today.
