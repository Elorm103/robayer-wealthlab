...
**Status:** Complete, including the mobile refinement pass below — verified locally, not yet committed pending user review of the mobile result specifically. Companion to `docs/homepage-ghanaian-redesign-plan.md` (the approved plan), `docs/design/ghanaian-design-language.md` (the durable rationale/philosophy reference), and `docs/design/brand-assets.md` (the asset inventory table — purpose, permitted use, and restrictions for every reusable Ghanaian design asset). Scope strictly visual/presentational — no backend, routing, API, database, Worker, CMS, or business-logic code was touched; no page content or copy changed.

## Addendum — Mobile refinement pass (third visual pass)

The desktop hero refinement (previous addendum below) was approved and deployed, but review on mobile surfaced five problems the desktop-only pass hadn't caught. This pass was scoped mobile-first per an explicit plan (see the "do not modify desktop unless necessary" framing) — most changes are genuinely mobile-only; the Kente weave and ambient-motion work necessarily touch the one shared `.kente-ribbon`/motion `@keyframes` used at both breakpoints, so desktop picks up the richer ribbon and the same extremely-subtle motion as an intended consequence, not extra desktop redesign:

- **Mobile hero recomposition** — `.hero--split .hero__visual` gets `order: -1` inside the existing `≤1199px` collapse breakpoint, so the illustration now leads the hero (visual hook → headline → subtitle → CTAs → founder credit) instead of trailing below the founder signature as an apparently-separate section. Purely visual reordering — DOM/reading order is unchanged. The illustration's mobile `max-width` was also trimmed from 260px to 220px so it reads as an accompanying visual, not a dominant block.
- **Fixed Gye Nyame/Sankofa mobile clipping** — root cause: `.adinkra-motif`'s `clamp(140px, 18vw, 220px)` floor doesn't shrink further below `18vw`'s crossover point, so on narrow viewports the motif pinned at a fixed 140px and clipped arbitrarily against `.section--motif`'s `overflow: hidden` edge. Added a `≤767px` override lowering the floor to `clamp(90px, 28vw, 150px)` and tightening the corner offsets, so both instances now show a deliberate corner-bleed instead of an accidental crop — visually confirmed for both Gye Nyame (Trust section) and Sankofa (Meet the Founder section).
- **Kente ribbon woven character** — added a `.kente-ribbon__weave` group of six small rotated "weft" rects, hand-placed and angled along the ribbon's existing curve, crossing the three flowing bands to suggest interlacing without becoming a literal high-fidelity weave (still an "elegant, simplified vector interpretation" per the design-language doc). Shares the ribbon's existing `kente-ribbon-drift` animation so the whole shape moves together.
- **True ambient motion** — four additions, all pure CSS `@keyframes`, none requiring JS: a very slow (34s) independent drift on the financial-context layer (background) distinct from the Nkyinkyim float (foreground) — a lightweight stand-in for parallax depth without a scroll listener; a gentle staggered pulse (10s, delays 0/-4s/-8s) on the three financial-context nodes; and the Nkyinkyim float extended to include a very subtle scale "breathe" (1 → 1.015) combined into the same keyframe as the existing translateY float (not a second competing animation). All confirmed via computed `animationName`/`animationDelay` checks (a 30s+ cycle isn't reliably visible in a static screenshot).
- **Re-verified**: mobile (375px) — illustration leads the hero, Gye Nyame/Sankofa crop deliberately, Kente weave visible, all new `animation-name`s present on their expected elements. Desktop (1400px) — split layout, Nkyinkyim illustration, and spacing unchanged apart from the intentionally-shared ribbon/motion updates. No console errors, no horizontal scroll at 375px/1400px. The global `@media (prefers-reduced-motion: reduce)` rule in `css/base.css` (unchanged) automatically covers all four new animations — confirmed by rule inspection, consistent with every prior pass in this project.
- **No new dependencies** — pure CSS/SVG, no JS added for the "parallax" effect (a layered-drift approximation instead of a scroll listener, matching this codebase's zero-build-step convention).

Not yet committed — holding per the user's request to review the mobile result before this (and the pending `v2.2.0` tag) gets finalized.

## Addendum — Hero refinement pass (second visual pass)

After the redesign below shipped, review of the live site found the hero "technically correct but too subtle" — centered, single-column, with modest decorative additions rather than a signature focal experience. This pass targeted the hero specifically, leaving every other section (Trust, Free Guide, Services, etc.) untouched apart from the Adinkra opacity change noted below:

- **Adopted `.hero--split`** — a pre-existing two-column layout modifier already proven on 7 other pages (`about`, `contact`, `community`, the eBook detail page, `product-detail-template`, `components.html`, `free-guide`) but never used on the homepage. No new layout CSS was written; the hero's markup now wraps its copy in `.hero__text` and adds a `.hero__visual` right column, collapsing to one centered column below 1199px exactly as the existing modifier already does elsewhere.
- **Removed the legacy floating circles** (`.hero__shape`/`.hero__shape--1/2/3` and their `hero-shape-drift` keyframe) — confirmed via repo-wide grep to be used only in `index.html`, so deleted outright rather than left dead. The hero now has one intentional focal point instead of several competing background shapes.
- **Nkyinkyim promoted to the hero's anchor illustration** — a large (~290px), soft, two-tone gradient shape (`--color-sika-gold` → `--color-sika-gold-dark`, both pre-existing tokens) with a blurred shadow duplicate (`feGaussianBlur`) for gentle depth, deliberately kept below full opacity (0.9) and free of stroke/glow/3D treatment so it reads as an editorial illustration rather than a badge or logo. This is documented as the one "anchor tier" Adinkra placement in `docs/design/ghanaian-design-language.md`, distinct from Gye Nyame/Sankofa's "watermark tier."
- **Kente ribbon rebuilt as filled gradient bands** — replaced the original thin `stroke-dasharray` outline with wide (`stroke-width: 20`) rounded gradient-stroked bands plus a blurred fold-shadow band beneath, reading as flowing fabric rather than an SVG line. The old `stroke-dashoffset` flow animation was replaced with a slower (26s) ambient drift, matching the site's existing slow-motion register.
- **Added a subtle Ghanaian financial-context accent** behind the Nkyinkyim illustration — a very low-opacity (9%) growth curve with connected nodes, plus a small scatter of abstract geometric accent marks (12% opacity) — never a duplicated Adinkra symbol, per the standing "no decorative wallpaper of named cultural symbols" rule. Both are deliberately static (no animation), so they stay unnoticed background texture rather than drawing the eye.
- **Raised Gye Nyame/Sankofa opacity from 6% to 9%** (`.adinkra-motif`'s base opacity) — within the approved 8–10% starting range, applied sitewide to both existing live instances (Trust section, Meet the Founder section) via the single shared class.
- **Re-verified**: desktop (1400px, `.hero--split` two-column grid confirmed), tablet (1100px, confirmed collapse to one centered column below the 1199px breakpoint), and mobile (375px) all screenshot-confirmed with no horizontal scroll (`scrollWidth === clientWidth` at all three widths) and no console errors. Confirmed programmatically: the Nkyinkyim gradient/shadow filter resolve correctly, the Kente ribbon's three gradient-stroked bands resolve correctly, `.adinkra-motif` computes to `opacity: 0.09` on both live instances, and the site's existing global `@media (prefers-reduced-motion: reduce)` rule (`css/base.css`) still automatically covers every animation introduced in this pass with no new reduced-motion code required.
- **No new dependencies, no new colour tokens** — the Kente ribbon's red-band gradient uses one `color-mix()` derivation of the existing `--color-kente-red` token (no `--color-kente-red-dark` token exists); every other colour is an existing token used as-is.

This is the final planned visual refinement for the homepage — see `docs/design/ghanaian-design-language.md`'s Illustration Style section for the explicit "homepage hero is now considered visually finished" note.
