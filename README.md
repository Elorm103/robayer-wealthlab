...
### Cache-busting (Version 3.6)

Every `/css/*.css` and `/js/**/*.js` reference in every page and Worker
route carries a `?v=<content-hash>` query string, generated by
`scripts/bump-asset-versions.mjs` — never hand-edited. If you change any
CSS or JS file, run:
...
before committing, so every reference to that file picks up its new
hash. One-time setup to have this happen automatically on every commit
instead:
...
See `docs/v3.6-cache-architecture-report.md` for why this replaced the
old manually-incremented `?v=m68` scheme.
